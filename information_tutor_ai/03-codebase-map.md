...
| `lib/api.ts` | Typed client for every gateway endpoint (Phase 1–5) with an `ApiError` type + bearer‑token handling (`setAuthToken`/`loadAuthToken`). |
...
| `components/auth-panel.tsx` | **Phase 5** — register / sign in / sign out; persists the JWT and sets the active user. |
...
|---|---|---|---|
| `gateway/app/main.py` | **gateway** | 8000 (`PORT`) | Full API: notebooks, sources, ask, solve, reveal, artifacts, notion, teaching/quizzes/papers/reports (Phase 2), revision/student/analytics/voice (Phase 3), connectors/multi-agent teaching/billing (Phase 4), **auth/`/metrics` + env‑gated bearer enforcement & quota 402 (Phase 5)**. |
| `rag/app/main.py` | **rag** | 8001 (`RAG_PORT`) | Mirrors the full Phase 1–5 route table via a clean `Route` list. |
...
| `store.py` | `InMemoryStudyLabStore` — default ephemeral store (Phase 1–5 collections incl. users). |
| `store_sqlite.py` | `SqliteStudyLabStore` — durable store with identical surface (Phase 1–5 tables incl. users); `make_store_from_env`. |
...
| `auth.py` | **Phase 5** — `AuthEngine`: register/login, PBKDF2 password hashing, stdlib HS256 JWT issue/verify; `hash_password` / `verify_password` / `make_auth_secret`; `AuthError`. |
...
| `docs/openapi.phase1.yml` | OpenAPI description of the API (Phase 0–5; title "StudyLab Phase 0-5 API"). |

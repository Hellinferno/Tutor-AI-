...
| File | What it is |
|---|---|
| `app/layout.tsx`, `app/page.tsx`, `app/styles.css` | App router root; main page (wraps the 10 panels in `NotebookProvider` + a notebook bar); styles. |
| `lib/api.ts` | Typed client for every gateway endpoint (Phase 1–3) with an `ApiError` type. |
| `lib/types.ts` | Response interfaces mirroring the Python models / shared types. |
| `lib/notebook-context.tsx` | `NotebookProvider` / `useNotebook` — shares the active notebook id + last attempt id across panels. |
| `components/notebook-bar.tsx` | Create / show the active notebook (header). |
...
|---|---|---|---|
| `gateway/app/main.py` | **gateway** | 8000 (`PORT`) | Full API: notebooks, sources, ask, solve, reveal, artifacts, notion, teaching/quizzes/papers/reports (Phase 2), **revision/student/analytics/voice (Phase 3)**. |
| `rag/app/main.py` | **rag** | 8001 (`RAG_PORT`) | Mirrors the full Phase 1–3 route table via a clean `Route` list. |
...
| `models.py` | Dataclasses for all phases: Phase 1 (`Notebook`, `Source`, `SourceChunk`, `SourceGuide`, `Citation`, `Solution`, `Artifact`, responses); **Phase 2** (`WhiteboardConcept`/`WhiteboardSession`, `QuizQuestion`/`Quiz`, `PaperSection`/`QuestionPaper`, `Attempt`, `AnswerKey`, `EvalReport`); **Phase 3** (`RevisionCard`, `Session`, `StudentProfile`, `TopicMastery`, `KnowledgeState`, `VoiceResult`); plus the `VerifyMethod`/`GroundingState`/`ArtifactType`/`QuestionType`/`AttemptSourceType`/`Difficulty` literals. |
| `store.py` | `InMemoryStudyLabStore` — default ephemeral store (Phase 1–3 collections). |
| `store_sqlite.py` | `SqliteStudyLabStore` — durable store with identical surface (Phase 1–3 tables); `make_store_from_env`. |
...
| `docs/openapi.phase1.yml` | OpenAPI description of the API (Phase 0–3; title "StudyLab Phase 0-3 API"). |
